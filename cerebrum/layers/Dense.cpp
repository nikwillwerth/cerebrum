...
Dense::Dense(Layer *inputLayer, std::size_t numOutputs) : Layer(inputLayer) {
    ...
    for(std::size_t i = 1; i < inputLayer->outputShape.size(); i++) {
        this->numInputs *= inputLayer->outputShape[i];
    }

    this->weights = Eigen::Tensor<double, 2>(int(numOutputs), this->numInputs);
    this->biases  = Eigen::Tensor<double, 2>(1, int(numOutputs));
...
Eigen::Tensor<double, 4> Dense::forward(Eigen::Tensor<double, 4> x) {
    this->previousX = x;
    ...
    Eigen::Tensor<double, 4, 0, long> reshapedOutput = TensorOps::reshape(output,          this->batchSize, this->numOutputs, 1, 1);
    Eigen::Tensor<double, 4, 0, long> reshapedBiases = TensorOps::reshape(broadcastBiases, this->batchSize, this->numOutputs, 1, 1);

    // output + biases
    return reshapedOutput + reshapedBiases;
}

Eigen::Tensor<double, 4> Dense::backward(Eigen::Tensor<double, 4> t) {
    ...
    this->dWeights = TensorOps::dot(TensorOps::transpose(reshapedT), reshapedX) / this->weights.constant(batchSize);
    this->dBiases  = TensorOps::sumKeepDims(reshapedT, 0) / this->biases.constant(batchSize);
    ...
    return TensorOps::reshape(output, this->batchSize, this->numInputs, 1, 1);
}
